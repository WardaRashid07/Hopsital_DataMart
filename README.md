...
- **Rows that fail Silver-layer validation are logged, not silently dropped.** `sp_load_silver` writes each rejected row — with the specific reason (invalid ID, missing required field, bad date, non-positive amount) and the original raw data as JSON — to `silver.rejected_rows` before the cleaning insert runs, so bad source data is visible and reviewable rather than disappearing.
...
- The pipeline currently loads from local CSV files; there's no incremental/streaming ingestion from the source systems it models. The source data has no timestamp or change-tracking mechanism to detect deltas, so this would require a different source setup, not just a pipeline change.
